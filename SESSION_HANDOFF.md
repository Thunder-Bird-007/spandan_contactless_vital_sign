...
- **2026-09-25** — Segment 28 (camera throughput improvement attempt, continues Segment 18 -- that doc's own "Honest verdict" section is unchanged). **No physical device attached this session** (`adb devices` empty, checked before and after all code changes) -- no fps/miss-rate/IoU numbers were measured or fabricated. Fixed Segment 18's own leading hypothesis: `OpticalFlowFaceTracker.track()`'s two per-skipped-frame `IntArray` allocations, now reused class-level buffers. Added a new, unmeasured candidate, off by default: `CroppedDetectionStrategy` (new file) runs real ML Kit detection on a small padded crop around the last known face box on skipped-detection frames instead of freezing/tracking it, with integer downscale support in the same code path; wired into `FaceAnalyzer.kt` (`useCroppedDetection` flag) and `ProfilingFaceAnalyzer.kt` (new `cropMs` phase + miss counter, so the measurement protocol is ready to run). 8 new unit tests for the pure crop-rect math (57/57 total pass). Found and flagged (not fixed, out of scope): `android.graphics.Rect`'s 4-arg constructor is a no-op under this project's plain-JUnit harness (`isReturnDefaultValues=true`), which means the existing `CoordinateMapperTest.kt` has been passing vacuously (compares zeroed rects against zeroed rects) -- production `CoordinateMapper.kt` itself is unaffected on a real device. This segment's own new test avoids the same trap via an explicit-field-assignment helper. Full detail, including the exact measurement protocol for the next device-having session: `android/docs/Segment28_Throughput_Improvement.md`.
