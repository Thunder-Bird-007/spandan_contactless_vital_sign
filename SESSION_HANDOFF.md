...
- **2026-09-25** — Segment 30 (MediaPipe Face Detector migration attempt, **NOT PROMOTED**), done entirely inside a new isolated sibling directory `android_segment30_mediapipe/` (applicationId `com.spandan.app.mediapipe`, installed alongside production `com.spandan.app` for direct A/B) per this segment's own task brief -- `android/` itself was never touched. Motivated by Google's own published MediaPipe Face Detector (BlazeFace short-range) benchmark, 2.94ms on a Pixel 6, vs. Segment 29's own measured ~19-25ms ML Kit cost. `com.google.mediapipe:tasks-vision:1.0.0` added (checked against the real maven-metadata.xml, not guessed); `FaceAnalyzer.kt`/`ProfilingFaceAnalyzer.kt` gained an OFF-by-default `useMediaPipeDetection` flag (same "evidence before promotion" convention as every prior candidate), plus a new `logMediaPipeVsMlKitComparison` flag that runs both detectors on the same frame to log IoU/center-distance -- the exact capability Segment 28's own doc had flagged as missing. **Real on-device crash found and fixed**: MediaPipe's Android packet creator rejects a raw YUV_420_888 `android.media.Image` through `MediaImageBuilder` (`UnsupportedOperationException: Android media image must use RGBA_8888 config`), forcing a per-frame YUV→ARGB8888 Bitmap conversion (`MediaPipeImageConverter.kt`, new, reusing `RoiPixelAverager`'s own stride-aware plane-indexing convention) that this migration's original zero-copy design had not planned for. **Real on-device measurement, same Galaxy A35**: MediaPipe measured **20.19fps** (909 frames/45.03s, mean bundled `detectMs` 94.88ms, range 81-128ms) -- slower than Segment 29's own ML Kit baseline (~23.7-23.84fps, ~19-25ms), not faster; a diagnostic split attributed ~71ms of the ~87ms bundled cost to the forced Bitmap conversion alone, leaving only ~16ms for the detector itself (roughly comparable to ML Kit, but unreachable through this integration path without removing that conversion tax). A 189-sample IoU/center-distance capture (ML Kit staying primary, MediaPipe running alongside for comparison only) found a real, systematic ~38px top-edge offset (MediaPipe's box starting consistently lower on the face than ML Kit's, IoU mean 0.617, center-distance mean 28.07px) -- independently confirmed by direct visual observation during testing that the forehead ROI was mis-tracking. `CoordinateMapper.kt` gained a new `mediaPipeSensorBoxToRotatedRect` (MediaPipe's box convention is the OPPOSITE of ML Kit's -- raw/unrotated sensor space, not rotated/upright -- confirmed by reading Google's own official mediapipe-samples source, since MediaPipe's own docs don't state this plainly). Writing this segment's own new unit tests surfaced and fixed a DEEPER version of Segment 28's already-flagged `Rect` testability gap -- `rotatedRectToSensorRect`/`sensorRectToRotatedRect` themselves built their return value via constructors that are ALSO no-ops under this project's plain-JUnit harness (not just the 4-arg form Segment 28 found), meaning those two functions were never actually exercisable by any unit test; fixed at the root via a `makeRect` helper matching `CroppedDetectionStrategy`'s own convention, which also, as a direct consequence, fixed `CoordinateMapperTest`'s existing round-trip test (previously passing only as a double-vacuous 0-equals-0 coincidence). 12 new unit tests (64 → 76 total pass). `MainActivity.kt` was temporarily swapped for each on-device capture and reverted, confirmed byte-identical to `android/`'s own copy via direct `diff` (this new directory is untracked, so a `git diff` isn't meaningful here) before the final commit. `useMediaPipeDetection`/`logMediaPipeVsMlKitComparison` stay `false` -- a clean negative result on both speed and accuracy, not an inconclusive one. Full detail, the diagnostic breakdown, and what would make a future re-attempt worth trying: `android_segment30_mediapipe/docs/Segment30_MediaPipe_Migration.md`.
- **2026-09-25** — Cut and published the `v1.2-android` GitHub release (`versionCode=3`, `versionName="1.2.0-throughput"`) from current `main`, since `v1.1-android` (tagged at `82f7f87`) predated Segments 28-29 and no longer reflected production `android/`'s real state (the throughput fix + promoted `minFaceSize`/Kalman defaults). Built fresh, 64/64 tests pass, `app-debug.apk` attached: https://github.com/Thunder-Bird-007/spandan_contactless_vital_sign/releases/tag/v1.2-android. Segment 30's MediaPipe work is noted in the release description but did not change this APK (never touched production `android/`).
- **2026-09-25** — Segment 31 (UI/UX: promote Branch 2's waveform to the primary "LIVE SIGNAL" card, as a genuine multi-cycle scrolling trace, + general aesthetic pass), user-requested after noticing the app's one prominent "LIVE SIGNAL" card was actually the raw, unfiltered green-channel signal (the shared input both branches read, not either branch's own output) while Branch 2's real output (the dicrotic-notch waveform, Segment 19) sat in a smaller, easy-to-miss secondary card below it. `SignalChartView.kt` and its raw-chart card deleted (confirmed unused first; user explicitly chose "replace entirely" over "keep as a smaller secondary strip" when asked). First pass promoted Branch 2's existing single ensemble-averaged-beat view into a tinted "hero" card (`bg_hero_card.xml`) directly under the camera; user follow-up, verbatim: "I don't want just a single cycle... I want it updating real time, showing 8-10 cycles just like a PPG monitor will do." Traced the root cause: `estimateVitalsAndMorphology.m`'s own port already computes a genuine multi-cycle CONTINUOUS pulse (post ABPF/Gaussian harmonic filtering + CHROM combine + polarity fix + uniform resample) before `EnsembleAverageBeats` collapses it into one averaged prototype -- that continuous signal was computed and then discarded, never surfaced. Fixed by adding `MorphologyWaveformEstimator.Estimate.continuousWaveform` (the same selected candidate's continuous pulse, tail-windowed to the last 8s = ~2000 samples at 250Hz, ~8-13 real cycles at a resting HR) alongside the existing (untouched, still unit-tested at a fixed 256 samples) single-beat `waveform` field; `WaveformView` now renders the multi-cycle trace and dropped its per-beat notch marker (doesn't map onto multiple cycles -- notch method/confidence stays visible via the existing status pill text, no information lost). 2 new unit tests for `continuousWaveform`'s size (66 total, up from 64). Found and fixed a real build break of its own making along the way: literal `--` inside several new XML comments (XML forbids this outright; `mergeDebugResources` hard-failed) -- fixed via em dash, matching this project's own existing XML comment convention. **Confirmed on the same Galaxy A35 under TWO different real conditions**, not just one good screenshot: a capture during a noisy moment (hand near face) showed a genuinely jagged trace alongside `conf 0.02` (amber pill) -- correct, honest behavior, not a bug, the same way a real monitor shows visible motion artifact rather than hiding it; a follow-up capture ~15s later with the subject holding still showed a clean, physiologically plausible multi-beat trace (consistent systolic/dicrotic double-peaks) alongside `ABPF comb · conf 0.40` (green pill) and 62bpm (62bpm over 8s ≈ 8.3 cycles, consistent with the ~13-14 peaks visible at ~2/cycle). Vitals card also gained small tinted heart/droplet icons; card backgrounds gained a subtle gradient instead of flat solid fill; a cosmetic scrim eases the camera-preview-to-cards transition. Full detail: `android/docs/Segment31_UI_UX_Branch2_LiveSignal.md`.
- **2026-09-25** — Segment 32 (port Branch 2 to iOS + the same multi-cycle live-signal UI, user-requested "do all the things for iOS too"). Discovery before starting: iOS had **no Branch 2 code at all** -- only Branch 1 (HR/SpO2) was ever ported (Segment 19's morphology work post-dated the original iOS PR and was never backported). User confirmed the full scope ("full Branch 2 port + live signal UI") when asked, rather than the cheaper UI-only alternative. Ported the whole DSP chain to Swift, same "read the Kotlin/MATLAB source directly before writing this" discipline as the original Branch 1 port: `PchipInterpolator`, `FixPolarity`, `MorphologyBandpassFilter` (reuses `BandpassFilter`'s existing `designButterworthBandpass`/`filtfilt`), `ResampleUniform`, `AdaptiveHarmonicFilter`/`HarmonicSelectiveGaussianFilter` (via a new shared `ComplexDFT` -- a direct O(n^2) DFT, matching this codebase's own `HeartRateFft.swift` precedent of avoiding Accelerate's power-of-2 FFT requirement, rather than porting JTransforms), `HarmonicFilterConfidenceGate`, `EnsembleAverageBeats`, `NotchDetectIEM`, and the `MorphologyWaveformEstimator` orchestrator -- built WITH the Android port's own Segment 31 `continuousWaveform` field from the start, not ported pre-Segment-31 and revised again. New `WaveformView.swift` (multi-cycle gradient trace) + a `MainViewController.swift` rewrite mirroring Android's own Segment 31 UI/UX pass (`SignalChartView.swift` deleted, replaced by a "LIVE SIGNAL" hero card; SF Symbol heart/drop icons on the vitals card). 9 new XCTest files (48 total). **HONEST STATUS, unchanged from the original iOS port's own convention**: no Mac/Xcode/iPhone was available this session -- the ONLY verification is CI (`.github/workflows/ios-build.yml`, macos-14 runner, Xcode 16.2), run via `gh workflow run`/`gh run watch`/direct API polling against a feature branch (`ios-branch2-live-signal`) BEFORE merging to `main`, not after. **The first CI run genuinely caught 3 failing tests** (a real, working safety net, not theater): `AdaptiveHarmonicFilterTests` had assumed a pure 8Hz tone produces no in-band FFT bin, false (finite-length DFTs leak into every bin); two `FixPolarityTests` had the flip-direction expectation backwards. Both root-caused to test-authoring mistakes, NOT port bugs -- the production Swift already matched the Kotlin source in both cases; fixed the tests, re-ran CI, got 48/48 passing plus a successful `build-ipa`, merged to `main` only then. `ios/README.md` updated with the same per-file port-status table convention the original port established, plus a new "Known risk area #6" flagging that Branch 2 (like Branch 1 before it) has zero on-device confidence, only CI/synthetic-signal confidence. New release: `v1.1-ios` (first iOS-specific release; `v1.0-demo` bundled both platforms). Segments 28-30's Android-specific throughput/MediaPipe work was NOT ported (ML-Kit/Vision-framework-specific, not portable, and Segment 30 was rejected anyway).
