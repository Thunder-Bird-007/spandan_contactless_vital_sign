% RUN_VIPL_PHONE_V1_BATCH Task L, Action 3 (v1 half). Runs the full,
% unmodified Segment 2-5 pipeline (extractROISignals -> detrendSignal ->
% bandpassClean -> chromCombine/posCombine -> fftHeartRate ->
% ratioOfRatios) against every VIPL-HR subject's v1 (baseline, stable,
% normal light, 1m) source2 (HUAWEI P9 phone) video -- see
% docs/VIPL_Scenario_Coverage.md for why this is a device-generalization
% check, not a range-strengthening exercise: v1/source2 already exists in
% the archive for all 107 subjects, but this project's existing
% run_vipl_integration_batch.m only ever processed source2 for the 12
% subjects (p84, p97-p107) that lack source1 entirely, using it as a
% fallback PRIMARY reading. This script instead processes source2 for
% EVERY subject that has it on disk, tagged with its own
% "VIPL_pX_v1_source2" subjectID, so v1/source2 becomes an independently
% addressable device-tagged row even for subjects that already have a
% v1/source1 row from the original batch -- this is what makes the
% source1-vs-source2 device-stratified comparison in
% run_vipl_device_stratified_eval.m possible.
%
% Subjects are discovered dynamically by checking which
% data/raw/VIPL-HR/pX/v1/source2/video.avi files actually exist on disk,
% rather than a hardcoded list -- extraction coverage for this scenario
% is expected to land at 107/107 per docs/VIPL_Scenario_Coverage.md
% Section 1, but this script does not assume that number and simply
% processes whatever was extracted. Subjects whose video file is not
% present are logged as "not extracted" and skipped without counting as
% a pipeline failure (a missing file here means Action 1/2's extraction
% pass did not reach that subject, not that this script's pipeline broke
% on it).
%
% Output naming mirrors run_vipl_integration_batch.m exactly (same
% column layout, same per-subject .mat/.png artifacts) but writes to its
% own segment4_hr_summary_vipl_phone_v1.csv /
% segment5_vipl_calibration_phone_v1.csv so it never touches the shared
% UBFC or original VIPL CSV files those other scripts own.
%
...
% the whole batch, same discipline as run_vipl_integration_batch.m.
...
hrCsvPath = fullfile(metricsRoot, 'segment4_hr_summary_vipl_phone_v1.csv');
...
for subjectNum = 1:maxSubjectNum
    ...
    if ~isfile(videoPath)
        notExtractedSubjects{end + 1} = subjectFolder;
        continue;
    end
    ...
    disp(['--- Processing ' subjectID ' (subject ' num2str(subjectNum) ' of ' num2str(maxSubjectNum) ') ---']);
    ...
    try
        ...
        sgtitle([subjectID ' -- Task L Phone v1 Filtering Sanity Check'], 'Interpreter', 'none');
        ...
        sgtitle([subjectID ' -- Task L Phone v1 Heart Rate Sanity Check'], 'Interpreter', 'none');
...
disp('--- Task L Phone v1 batch complete ---');
disp(['Subjects with v1/source2 extracted and attempted: ' num2str(numAttempted)]);
disp(['Subjects not extracted (skipped, not a failure): ' num2str(numel(notExtractedSubjects))]);
...
if numUsable < 3
    ...
    spo2CsvPath = fullfile(metricsRoot, 'segment5_vipl_calibration_phone_v1.csv');
...
disp('--- Task L Phone v1 batch script complete ---');
